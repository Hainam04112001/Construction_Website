...
<body>
    ...
    <div class="projectNav">
        <?php
        $projectNav=mysqli_query($conn,"Select * from projects_category");
        $projectVar = 0;
        while($proDataNav=mysqli_fetch_assoc($projectNav)){
            ?>
                <span id="<?php echo $proDataNav['category_name'];?>Nav" class="<?php echo $projectVar==0?'activeProject':'';?>" onclick="showProject('#<?php echo $proDataNav['category_name'];?>','#<?php echo $proDataNav['category_name'];?>Nav');"><?php echo $proDataNav['category_name'];?></span>
            <?php
            $projectVar=1;
        }
        ?>
         ...
         <!-- Projects -->
    <?php
        $categoryNav=mysqli_query($conn,"Select * from projects_category");
        $projectVar=0;
        while($dataNav=mysqli_fetch_assoc($categoryNav)){
            ?>
                <div class="project <?php echo $projectVar==0?'':'hideClass'?>" id="<?php echo $dataNav['category_name'];?>">
                    <?php
                    ...
                    if(mysqli_num_rows($project)){
                        while($projectImgData=mysqli_fetch_assoc($project)){
                            ...
                            ?>
                                <img src="projectImg/<?php echo $imgarr[0];?>" alt="Not Found" onclick="showPopup('#popup<?php echo $projectImgData['id']?>');">
                            
                                <!-- Project Popup -->
                        <div class="projectPopUp" id="popup<?php echo $projectImgData['id'];?>">
                                    <div class="projectPopUpContent">
                                        <h3><?php echo $projectImgData['project_name']; ?></h3>
                                        <p><?php echo $projectImgData['details']; ?></p>
                                    </div>
                                    <div class="projectPopUpImg" id="project_popup_img<?php echo $projectImgData['id'];?>" style="background-image:url('projectImg/<?php echo $imgarr[0];?>');background-size:100% 100%');">
                                        <i class="fa fa-times-circle fa-2x" aria-hidden="true" id="closeProject"onclick="closePopup('#popup<?php echo $projectImgData['id']?>');"></i>
                                        <div>
                                            <?php
                                            for($i=0;$i<count($imgarr);$i++){
                                                ?>
                                                    <img src="projectImg/<?php echo $imgarr[$i];?>" alt="Not Found" onclick="changeBackground('project_popup_img<?php echo $projectImgData['id'];?>','<?php echo $imgarr[$i];?>');">                                                
                                                <?php
                                            }
    ...
    <script>
        function showProject(showProject,showProjectNav) {
            <?php
                $hideNav=mysqli_query($conn,"Select * from projects_category");
